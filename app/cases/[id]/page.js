...
export default async function CaseDetailPage({ params }) {
  ...
  if (!caseRow[0]) return <div className="p-8 text-white">Case not found.</div>;
  ...
  return (
    <main className="min-h-screen bg-[#0a0a0f] text-white">
      <style>{`
        ...
        .gold { color: #c9a84c; }
        .card { background: rgba(255,255,255,0.04); border: 1px solid rgba(201,168,76,0.15); }
      ...
      <div className="max-w-3xl mx-auto p-6">
        <div className="card rounded-2xl p-6 mb-6">
          <div className="flex items-start justify-between mb-4">
            <div>
              <h1 className="hf text-2xl font-black text-white mb-1">{c.title}</h1>
              <p className="bf text-gray-400 text-sm">{c.court}{c.courtType ? ` · ${c.courtType}` : ""}</p>
            </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm bf">
            ...
            {c.nextDate && <div><span className="text-gray-500 text-xs">Next Hearing</span><div className="text-yellow-400 font-semibold">{c.nextDate}</div></div>}
            {c.caseType && <div><span className="text-gray-500 text-xs">Type</span><div className="text-white">{c.caseType}</div></div>}
          </div>
          {c.notes && <p className="bf text-gray-400 text-sm mt-4 border-t border-white/5 pt-4">{c.notes}</p>}
          {client[0] && (
            <div className="mt-4 border-t border-white/5 pt-4">
              <span className="bf text-gray-500 text-xs">Client: </span>
              <span className="bf text-white text-sm font-semibold">{client[0].name}</span>
              {client[0].phone && <span className="bf text-gray-400 text-sm"> · {client[0].phone}</span>}
            </div>
          )}
        </div>
        ...
        <div className="grid grid-cols-3 gap-4 mb-6">
          {[
            { label: "Charged", value: `₹${totalCharged.toLocaleString("en-IN")}`, color: "text-white" },
            { label: "Received", value: `₹${totalPaid.toLocaleString("en-IN")}`, color: "text-green-400" },
            { label: "Due", value: `₹${totalDue.toLocaleString("en-IN")}`, color: totalDue > 0 ? "text-red-400" : "text-green-400" },
          ].map((s, i) => (
            <div key={i} className="card rounded-xl p-4 text-center">
              <div className={`hf text-xl font-black ${s.color}`}>{s.value}</div>
              <div className="bf text-gray-500 text-xs mt-1">{s.label}</div>
            </div>
          ))}
        </div>
        ...
        <div className="mb-8">
          <h2 className="hf text-xl font-bold mb-4">Hearings</h2>
          {allHearings.length === 0 ? (
            <p className="bf text-gray-500 text-sm mb-4">No hearings recorded yet.</p>
          ) : (
            <div className="flex flex-col gap-3 mb-4">
              {allHearings.sort((a, b) => b.date.localeCompare(a.date)).map((h) => (
                <div key={h.id} className="card rounded-xl p-4">
                  <div className="flex items-center justify-between mb-1">
                    <span className="bf text-yellow-400 font-semibold text-sm">{h.date}</span>
                    {h.nextDate && <span className="bf text-xs text-gray-400">Next: {h.nextDate}</span>}
                  </div>
                  {h.outcome && <p className="bf text-gray-300 text-sm">{h.outcome}</p>}
                  {h.notes && <p className="bf text-gray-500 text-xs mt-1">{h.notes}</p>}
                </div>
              ))}
            </div>
          )}
        ...
        <div>
          <h2 className="hf text-xl font-bold mb-4">Fee Ledger</h2>
          {allFees.length === 0 ? (
            <p className="bf text-gray-500 text-sm mb-4">No fee entries yet.</p>
          ) : (
            <div className="flex flex-col gap-3 mb-4">
              {allFees.map((f) => (
                <div key={f.id} className="card rounded-xl p-4 flex items-center justify-between">
                  <div>
                    <div className="bf text-white font-semibold text-sm">₹{f.amount.toLocaleString("en-IN")} charged</div>
                    <div className="bf text-green-400 text-xs">₹{f.paid.toLocaleString("en-IN")} received · ₹{f.due.toLocaleString("en-IN")} due</div>
                    {f.note && <div className="bf text-gray-500 text-xs mt-0.5">{f.note}</div>}
                  </div>
                  <div className="text-right">
                    <div className="bf text-gray-500 text-xs">{f.mode}</div>
                    {f.paymentDate && <div className="bf text-gray-500 text-xs">{f.paymentDate}</div>}
                  </div>
                </div>
              ))}
            </div>
